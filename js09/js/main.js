...
function draw_grid(){
	...
	if(orientation_now==="PORTRAIT"){
		for(let i=0; i<W+1; i++){
			ctx.beginPath();
			ctx.moveTo(i*SIZE, Y0+0);
			ctx.lineTo(i*SIZE, Y0+H*SIZE);
			ctx.strokeStyle = color.grid;
			ctx.stroke();
		}
		for(let i=0; i<H+1; i++){
			ctx.beginPath();
			ctx.moveTo(0     , Y0+i*SIZE);
			ctx.lineTo(W*SIZE, Y0+i*SIZE);
			ctx.strokeStyle = color.grid;
			ctx.stroke();
		}
	}else{
...
function get_SIZE(){
	...
	if(window.innerWidth > window.innerHeight){
		...
		if(rel>1.3 ){
			console.log("ok", 1.3);
			SIZE = Math.trunc(window.innerWidth/15);
			W = 15;
			H = 15;			
		}
		...
		if(rel>1.7 ){
			...
			W = 15;
			H = 20;			
			// SIZE = Math.trunc(window.innerWidth/30);
			// W = 30;
			// H = 40;						
		}
