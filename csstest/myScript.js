...
function usarPlacaje(pkmDefensor){
   ...
   var fail=1;
   var elAtaque="Placaje";
   
   myDice=D6();
   if(myDice===pkm1CR){
      pkm1esCR="si";
      pkm1AT = pkm1ATextra + 2*D6();
      pkm1DF = pkm1DFextra + 2*D6();
      pkm1esFail = "no";
   }else{
      pkm1esCR="no";
      pkm1AT = pkm1ATextra +   D6();
      pkm1DF = pkm1DFextra +   D6();
      if(myDice<=fail){pkm1esFail="si";}else{pkm1esFail="no";}
   }

   myDice=D6();
   if(myDice===pkm2CR){
      pkm2esCR="si";
      pkm2AT = pkm2ATextra + 2*D6();
      pkm2DF = pkm2DFextra + 2*D6();
      pkm2esFail = "no";
   }else{
      pkm2esCR="no";
      pkm2AT = pkm2ATextra +   D6();
      pkm2DF = pkm2DFextra +   D6();
      if(myDice<=fail){pkm2esFail="si";}else{pkm2esFail="no";}
   }
   ...
   if(pkmDefensor===pkm1Name){
      ...
      if(pkm2paralizado==="si"){
         if(D6()>=5){
            pkm2paralizado="no";
            if(pkm2esCR==="si")pkm2paralizado="no";
         }else{
      ...
      if(pkm2paralizado==="no"){
         if(pkm2esFail==="si"){
            document.getElementById("infoGreen").innerHTML=
            pkmAtacante + " usó "+elAtaque+", pero falló.";
         }else{            
            if(pkm1paralizado==="si"){
               document.getElementById("infoGreen").innerHTML=
               pkmAtacante + " usó "+elAtaque+".";
               pkm1HP--;
            }
            if(pkm1paralizado==="no"){
               if(pkm2AT>pkm1DF){
                  ...
                  document.getElementById("infoGreen").innerHTML=
                  pkmAtacante + " usó "+elAtaque+", pero "+pkmDefensor+" lo bloqueó.";
               }
            }
         }
      }
   ...
   if(pkmDefensor===pkm2Name){
      ...
      if(pkm1paralizado==="si"){
         if(D6()>=5){
            pkm1paralizado="no";
            if(pkm1esCR==="si")pkm1paralizado="no";
         }else{
            document.getElementById("infoGreen").innerHTML= 
