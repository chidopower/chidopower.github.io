...
function load_vars(){
	...
	hero.y = 0.75*H;
	hero.r = SIZE/2;
	hero.imgR = img_hero_R;
	hero.imgL = img_hero_L;
	...
	button_pause.w = SIZE;
	button_pause.h = SIZE;
	button_pause.img = img_button_pause;
...
function update_and_draw(){
	...
	// if(state_old===GAMEOVER && state===WRONG_ORIENT && orientation_is_ok){
		// set_canvas();		
		// state_old = state;
		// state = GAMEOVER;
		// frame = 0;
...
function HeroLikeLink(){
	...
	this.draw = function(){
		if(this.dir==="R")
			ctx.drawImage(this.imgR, this.x-this.r, this.y-this.r, 2*this.r, 2*this.r);
		else
			ctx.drawImage(this.imgL, this.x-this.r, this.y-this.r, 2*this.r, 2*this.r);
	};
...
function spawn_stars(){
	...
	stars[0].img = img_star_black;
	stars[1].img = img_star_black;
...
function spawn_ball(){
	...
	balls[balls.length-1].img = img_ball;
	if(irand(1,2)===1)
...
function click_listener(e){
	...
	//canvas.removeEventListener('click',click_listener, false );
}
...
// canvas.addEventListener('touchload_vars', function(){console.log("H");hero.dy = -hero.dd*SIZE;}, false);
